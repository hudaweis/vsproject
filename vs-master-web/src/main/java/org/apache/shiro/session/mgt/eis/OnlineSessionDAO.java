...
public class OnlineSessionDAO extends EnterpriseCacheSessionDAO {
	 ...
	 * 上次同步数据库的时间戳
	 */
	private static final String LAST_SYNC_DB_TIMESTAMP = OnlineSessionDAO.class
	...
	protected Session doReadSession(Serializable sessionId) {
		UserOnline userOnline = userOnlineService.findOne(String
				.valueOf(sessionId));
		if (userOnline == null) {
			return null;
		}
	...
	public void syncToDb(OnlineSession session) {
		...
		Date lastSyncTimestamp = (Date) session
		...
		if (lastSyncTimestamp != null) {
			...
			long deltaTime = session.getLastAccessTime().getTime()
					- lastSyncTimestamp.getTime();
			if (deltaTime < dbSyncPeriod) { // 时间差不足 无需同步
				needSync = false;
			}
			boolean isGuest = session.getUserId() == null
					|| session.getUserId() == 0L;
		...
		session.setAttribute(LAST_SYNC_DB_TIMESTAMP,
	...
	protected void doDelete(Session session) {
		...
		if (onlineSession.getAttribute(ShiroConstants.ONLY_CLEAR_CACHE) == null) {
			try {
				userOnlineService
						.offline(String.valueOf(onlineSession.getId()));
			} catch (Exception e) {
				// 即使删除失败也无所谓
			}
		}
