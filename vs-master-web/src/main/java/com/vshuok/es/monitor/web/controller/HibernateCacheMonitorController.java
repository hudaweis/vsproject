...
public class HibernateCacheMonitorController extends BaseController {
	...
	public void setCommonData(Model model) {
		Statistics statistics = HibernateUtils.getSessionFactory(em)
				.getStatistics();
	...
	public String index(Model model) {
		...
		Map<String, Object> properties = new TreeMap<String, Object>(em
	...
	public String evictCollection(
		...
		boolean collectionRoleNamesEmpty = ArrayUtils
				.isEmpty(collectionRoleNames);
		boolean collectionEntityIdsEmpty = ArrayUtils
				.isEmpty(collectionEntityIds);
		...
		if (collectionRoleNamesEmpty && collectionEntityIdsEmpty) {
			...
			for (String collectionRoleName : collectionRoleNames) {
				for (Serializable collectionEntityId : collectionEntityIds) {
					cache.evictCollection(collectionRoleName,
							collectionEntityIds);
				}
			}
		}
	...
	private void setMemoryInfo(Model model) {
		...
		String[] secondLevelCacheRegionNames = statistics
		...
		for (String secondLevelCacheRegionName : secondLevelCacheRegionNames) {
			...
			totalMemoryCount += secondLevelCacheStatistics
					.getElementCountInMemory();
			totalDiskCount += secondLevelCacheStatistics
