package com.vshuok.es.showcase.excel.web.controller;
...
import com.vshuok.es.showcase.excel.service.ExcelDataService;
import com.vshuok.es.showcase.excel.web.controller.entity.ExcelDataType;
...
public class ExcelController extends BaseCRUDController<ExcelData, Long> {
	 ...
	 */
	@RequestMapping("/init")
	public String initOneMillionData(@CurrentUser User user,
			RedirectAttributes redirectAttributes) {
		getExcelDataService().initOneMillionData(user);
		redirectAttributes.addFlashAttribute(Constants.MESSAGE,
				"任务已经提交，正在执行，数据量比较大，执行完成后会在页面右上角的“我的通知”中通知你");
		return redirectToUrl(null);
	}

	@RequestMapping(value = "/import", method = RequestMethod.GET)
	public String showImportExcelForm(@RequestParam("type") ExcelDataType type,
			Model model) {
	...
	public String importExcel(@CurrentUser User user,
		...
		redirectAttributes.addFlashAttribute(Constants.MESSAGE,
				"导入任务已提交，任务执行完成后会在页面右上角的“我的通知”中通知你");
		return redirectToUrl(null);
	}
	...
	private boolean canImport(final MultipartFile file, final Model model) {
		...
		if (!(filename.endsWith("csv") || filename.endsWith("xls") || filename
				.endsWith("xlsx"))) {
	...
	public String exportExcel(@CurrentUser User user, Searchable searchable,
		...
		case excel2003_sheet:
			getExcelDataService().exportExcel2003WithOneSheetPerWorkBook(user,
					contextRootPath, searchable);
			break;
		case excel2003_xml:
			getExcelDataService().exportExcel2003WithXml(user, contextRootPath,
					searchable);
			break;
		case excel2003_usermodel:
			getExcelDataService().exportExcel2003WithUsermodel(user,
					contextRootPath, searchable);
			break;
		case excel2007:
			getExcelDataService().exportExcel2007(user, contextRootPath,
					searchable);
			break;
		default:
			// none
		...
		redirectAttributes.addFlashAttribute(Constants.MESSAGE,
				"导出任务已提交，任务执行完成后会在页面右上角的“我的通知”中通知你");
		return redirectToUrl(null);
	}
