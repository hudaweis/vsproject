...
import com.vshuok.es.showcase.product.entity.Product;
...
public class ProductController extends BaseCRUDController<Product, Long> {
	...
	public String listByCategory(Searchable searchable,
		...
		if (category != null) {
			...
			searchable.addSearchFilter("category.id", SearchOperator.eq,
					category.getId());
		}
		return super.list(searchable, model);
	}

	// @PathVariable("category") 会把数据放到request
	// attribute中，如果想绑定到spring:form上，必须把数据放到model中
	...
	@RequestMapping(value = { "/category-{categoryId}/{m}/update",
			"/{m}/update" }, method = RequestMethod.GET)
	public String showUpdateForm(@PathVariable("m") Product product, Model model) {
		return super.showUpdateForm(product, model);
	}

	@RequestMapping(value = { "/category-{categoryId}/{id}/update",
			"/{id}/update" }, method = RequestMethod.POST)
	public String update(Model model,
		...
		return super
				.update(model, product, result, backURL, redirectAttributes);
	}

	@RequestMapping(value = { "/category-{categoryId}/{id}/delete",
			"/{id}/delete" }, method = RequestMethod.GET)
	public String showDeleteForm(@PathVariable("id") Product product,
			Model model) {
		return super.showDeleteForm(product, model);
	}

	@RequestMapping(value = { "/category-{categoryId}/{id}/delete",
			"/{id}/delete" }, method = RequestMethod.POST)
