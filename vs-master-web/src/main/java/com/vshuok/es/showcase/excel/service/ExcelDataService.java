package com.vshuok.es.showcase.excel.service;
...
import com.vshuok.es.common.utils.FileCharset;
import com.vshuok.es.maintain.editor.web.controller.utils.CompressUtils;
import com.vshuok.es.maintain.notification.service.NotificationApi;
import com.vshuok.es.showcase.excel.entity.ExcelData;
import com.vshuok.es.showcase.excel.repository.ExcelDataRepository;
...
public class ExcelDataService extends BaseService<ExcelData, Long> {
	...
	public void importExcel2003(final User user, final InputStream is) {
		...
		try {
			...
			// 添加监听器
			req.addListenerForAllRecords(new Excel2003ImportListener(proxy,
					dataList, batchSize));
			// 创建事件工厂
	...
	public void importExcel2007(final User user, final InputStream is) {
		...
		try {
			...
			ContentHandler handler = new Excel2007ImportSheetHandler(proxy,
					dataList, batchSize);
	...
	@Async
	public void exportCvs(final User user, final String contextRootPath,
			...
			while (true) {
				...
				do {
					...
					if (!searchable.containsSearchKey("id_in")) {
						searchable.addSearchFilter("id", SearchOperator.gt,
								maxId);
					}
					page = findAll(searchable);
					for (ExcelData data : page.getContent()) {
						out.write(String.valueOf(data.getId()).getBytes(
								encoding));
						out.write(separator.getBytes(encoding));
						out.write((data.getContent()).replace(separator, ";")
								.getBytes(encoding));
						out.write("\n".getBytes(encoding));
	...
	/**
	 * 写多个workbook 1、给用户一个vbs 脚本合并 2、给用户写一个c#程序合并 不想这么麻烦 需要时再写吧，还不如直接让用户装office
	 * 2007 更简单。
	 * 
	 * @param user
	...
	public void exportExcel2003WithOneSheetPerWorkBook(final User user,
			final String contextRootPath, final Searchable searchable) {
		int workbookCount = 0;
		...
		try {
			...
			while (true) {
				workbookCount++;
				String fileName = generateFilename(user, contextRootPath,
						workbookCount, extension);
			...
			if (workbookCount > 1 || needCompress(new File(fileName))) {
				fileName = fileName.substring(0, fileName.lastIndexOf("_"))
						+ ".zip";
				// 去掉索引
				compressAndDeleteOriginal(fileName,
						workbookFileNames.toArray(new String[0]));
			} else {
	 ...
	 */
	@Async
	public void exportExcel2003WithXml(final User user,
			final String contextRootPath, final Searchable searchable) {
		...
		try {
			...
			while (true) {
				...
				out.write(sheetHeader.replace("{sheetName}",
						"Sheet" + totalSheets).getBytes(templateEncoding));
				...
				do {
					...
					for (ExcelData data : page.getContent()) {
						out.write(rowTemplate
								...
								.replace("{content}", data.getContent())
								.getBytes(templateEncoding));
						maxId = Math.max(maxId, data.getId());
						totalRows++;
					}
					// clear entity manager
	...
	/**
	 * excel 2003 不支持大数据量 每个sheet最多65536行(因为是usermodel模型，数据先写到内存 最后flush出去
	 * 不支持大数据量导出)
	 * 
	 * @param user
	 * @param contextRootPath
	 ...
	 */
	@Async
	public void exportExcel2003WithUsermodel(final User user,
			final String contextRootPath, final Searchable searchable) {
		int perSheetRows = 60000; // 每个sheet 6w条
		int totalRows = 0;
		Long maxId = 0L;
	...
	/**
	 * 支持大数据量导出 excel 2007 每个sheet最多1048576行
	 * 
	 * @param user
	 * @param contextRootPath
	 * @param searchable
	 ...
	 * @return
	 */
	private String generateFilename(final User user,
			final String contextRootPath, final String extension) {
		return generateFilename(user, contextRootPath, null, extension);
	}

	private String generateFilename(final User user,
			final String contextRootPath, Integer index, final String extension) {
		String path = FilenameUtils.concat(contextRootPath, storePath);
		path = FilenameUtils.concat(path, user.getUsername());
		path = FilenameUtils.concat(path, EXPORT_FILENAME_PREFIX + "_"
