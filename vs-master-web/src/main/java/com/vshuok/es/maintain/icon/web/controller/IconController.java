package com.vshuok.es.maintain.icon.web.controller;
...
import com.vshuok.es.maintain.icon.entity.IconType;
import com.vshuok.es.maintain.icon.service.IconService;
...
public class IconController extends BaseCRUDController<Icon, Long> {
	...
	@Override
	public String create(Model model, @Valid @ModelAttribute("m") Icon icon,
			BindingResult result, RedirectAttributes redirectAttributes) {
		throw new RuntimeException("discarded method");
	}

	@RequestMapping(value = "{type}/create", method = RequestMethod.GET)
	public String showCreateForm(@PathVariable(value = "type") IconType type,
			Model model) {
		Icon icon = new Icon();
		icon.setType(type);
	...
	@Override
	public String update(Model model, @Valid @ModelAttribute("m") Icon icon,
	...
	public String update(HttpServletRequest request, Model model,
		...
		String view = super.update(model, icon, result, backURL,
				redirectAttributes);
		genIconCssFile(request);
		return view;
	}
	...
	public String genIconCssFile(HttpServletRequest request) {
		...
		Searchable searchable = Searchable.newSearchable().addSearchParam(
				"type_in",
		...
		for (Icon icon : iconList) {
			...
			if (icon.getType() == IconType.upload_file) {
				cssList.add(String.format(uploadFileTemplate,
						icon.getIdentity(), ctx, icon.getImgSrc(),
						icon.getWidth(), icon.getHeight(), icon.getStyle()));
				continue;
			}

			if (icon.getType() == IconType.css_sprite) {
				cssList.add(String.format(cssSpriteTemplate,
						icon.getIdentity(), ctx, icon.getSpriteSrc(),
						icon.getLeft(), icon.getTop(), icon.getWidth(),
		...
		try {
			ServletContextResource resource = new ServletContextResource(sc,
					iconClassFile);
	...
	public Object validate(@RequestParam("fieldId") String fieldId,
		...
		if ("identity".equals(fieldId)) {
			...
			if (icon == null
					|| (icon.getId().equals(id) && icon.getIdentity().equals(
							fieldValue))) {
				// 如果msg 不为空 将弹出提示框
