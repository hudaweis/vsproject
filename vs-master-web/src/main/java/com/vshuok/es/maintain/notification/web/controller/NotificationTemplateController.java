...
public class NotificationTemplateController extends
		BaseCRUDController<NotificationTemplate, Long> {
	...
	protected boolean hasError(NotificationTemplate m, BindingResult result) {
		...
		NotificationTemplate template = getNotificationTemplateService()
				.findByName(m.getName());
		if (template == null
				|| (template.getId().equals(m.getId()) && template.getName()
						.equals(m.getName()))) {
			// success
		} else {
	...
	public Object validate(@RequestParam("fieldId") String fieldId,
		...
		if ("name".equals(fieldId)) {
			NotificationTemplate template = getNotificationTemplateService()
					.findByName(fieldValue);
			if (template == null
					|| (template.getId().equals(id) && template.getName()
							.equals(fieldValue))) {
				// 如果msg 不为空 将弹出提示框
