...
import com.vshuok.es.sys.user.service.PasswordService;
import com.vshuok.es.sys.user.service.UserLastOnlineService;
...
public class LoginUserController extends BaseController<User, Long> {
	...
	public String viewInfo(@CurrentUser User user, Model model) {
		...
		model.addAttribute("lastOnline",
				userLastOnlineService.findByUserId(user.getId()));
	...
	public String updateInfo(@CurrentUser User user,
		...
		if (mobilePhoneNumber == null
				|| !mobilePhoneNumber.matches(User.MOBILE_PHONE_NUMBER_PATTERN)) {
		...
		User mobilePhoneNumberDbUser = userService
				.findByMobilePhoneNumber(mobilePhoneNumber);
		if (mobilePhoneNumberDbUser != null
				&& !mobilePhoneNumberDbUser.equals(user)) {
