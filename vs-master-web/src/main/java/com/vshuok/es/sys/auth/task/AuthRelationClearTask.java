package com.vshuok.es.sys.auth.task;
...
public class AuthRelationClearTask {
		...
		do {
			...
			try {
				AuthRelationClearTask authRelationClearService = (AuthRelationClearTask) AopContext
						.currentProxy();
				authRelationClearService.doClear(authPage.getContent(),
						allRoleIds);
			} catch (Exception e) {
	...
	public void doClear(Collection<Auth> authColl, Set<Long> allRoleIds) {
		for (Auth auth : authColl) {
			...
			boolean hasRemoveAny = auth.getRoleIds().retainAll(allRoleIds);
			if (hasRemoveAny) {
	...
	private Set<Long> findAllRoleIds() {
		return Sets.newHashSet(Lists.transform(roleService.findAll(),
				new Function<Role, Long>() {
					@Override
