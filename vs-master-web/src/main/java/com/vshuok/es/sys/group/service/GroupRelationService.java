...
public class GroupRelationService extends BaseService<GroupRelation, Long> {
	...
	public void appendRelation(Long groupId, Long[] organizationIds) {
		...
		for (Long organizationId : organizationIds) {
			...
			GroupRelation r = getGroupRelationRepository()
					.findByGroupIdAndOrganizationId(groupId, organizationId);
			if (r == null) {
				r = new GroupRelation();
	...
	public void appendRelation(Long groupId, Long[] userIds,
			Long[] startUserIds, Long[] endUserIds) {
		...
		if (!ArrayUtils.isEmpty(startUserIds)) {
			for (int i = 0, l = startUserIds.length; i < l; i++) {
				...
				GroupRelation r = getGroupRelationRepository()
						.findByGroupIdAndStartUserIdLessThanEqualAndEndUserIdGreaterThanEqual(
								groupId, startUserId, endUserId);

				if (r == null) {
					// 删除范围内的
					getGroupRelationRepository().deleteInRange(startUserId,
							endUserId);
					r = new GroupRelation();
					r.setGroupId(groupId);
