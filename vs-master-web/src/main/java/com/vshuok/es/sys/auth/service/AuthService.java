package com.vshuok.es.sys.auth.service;

import com.vshuok.es.common.service.BaseService;
import com.vshuok.es.sys.auth.entity.Auth;
import com.vshuok.es.sys.auth.repository.AuthRepository;
...
public class AuthService extends BaseService<Auth, Long> {
	...
	public void addOrganizationJobAuth(Long[] organizationIds, Long[][] jobIds,
			Auth m) {
	...
	private void addOrganizationJobAuth(Long organizationId, Long jobId, Auth m) {
		...
		Auth auth = getAuthRepository().findByOrganizationIdAndJobId(
	...
	public Set<Long> findRoleIds(Long userId, Set<Long> groupIds,
			Set<Long> organizationIds, Set<Long> jobIds,
			Set<Long[]> organizationJobIds) {
		return getAuthRepository().findRoleIds(userId, groupIds,
