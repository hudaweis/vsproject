package com.vshuok.es.sys.auth.web.controller;
...
import com.vshuok.es.sys.auth.entity.AuthType;
...
public class AuthController extends BaseCRUDController<Auth, Long> {
	...
	protected void setCommonData(Model model) {
		...
		model.addAttribute("roles",
				roleService.findAllWithNoPageNoSort(searchable));
	}
	...
	public String create(Model model, @Valid @ModelAttribute("m") Auth m,
			BindingResult result, RedirectAttributes redirectAttributes) {
		throw new RuntimeException("discard method");
	}

	@RequestMapping(value = "{type}/create", method = RequestMethod.GET)
	public String showCreateFormWithType(@PathVariable("type") AuthType type,
			Model model) {
		Auth auth = new Auth();
		auth.setType(type);
	...
	public String createWithType(
		...
		} else if (m.getType() == AuthType.user_group
				|| m.getType() == AuthType.organization_group) {
