	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="showcase:product:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/product/product/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/showcase/product/product/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="showcase:product:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/product/product/${m.id}/update?BackURL=<es:BackURL/>">
						<i class="icon-edit"></i> 修改
				</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="showcase:product:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/product/product/${m.id}/delete?BackURL=<es:BackURL/>">
	...
	<form:form id="editForm" method="post" commandName="m"
		...
		<div class="control-group">
			...
			<div class="controls input-append date">
				<form:input path="beginDate" data-format="yyyy-MM-dd hh:mm:ss"
					placeholder="例如2013-02-07 11:11:11" />
		...
		<div class="control-group">
			...
			<div class="controls input-append date">
				<form:input path="endDate" data-format="yyyy-MM-dd hh:mm:ss"
					placeholder="例如2013-02-07 11:11:11" />
				<span class="add-on"><i data-time-icon="icon-time"
					data-date-icon="icon-calendar"></i></span>
			</div>
