...
<table id="table"
	class="sort-table move-table table table-bordered table-hover"
	data-move-url-prefix="${ctx}/showcase/product/category"
	data-async="true">
	<thead>
		<tr>
		...
		<c:forEach items="${page.content}" var="m" varStatus="status">
			<tr id="${m.id}">
				<td class="check"><input type="checkbox" name="ids"
					value="${m.id}"></td>
				<td><a href="${ctx}/showcase/product/category/${m.id}">
						${m.id} </a></td>
				<td>${m.name}</td>
				<td>${m.weight}<es:movable page="${page}"
