...
<div data-table="table" class="panel">
	...
	<ul class="nav nav-tabs">
		<li ${empty param['search.show_eq'] ? 'class="active"' : ''}><a
			href="${ctx}/showcase/product/product"> <i class="icon-table"></i>
				所有产品列表
		</a></li>
		<li ${param['search.show_eq'] eq 'true' ? 'class="active"' : ''}>
			<a href="${ctx}/showcase/product/product?search.show_eq=true"> <i
				class="icon-table"></i> 可显示的产品列表
		</a>
		</li>
		<li ${param['search.show_eq'] eq 'false' ? 'class="active"' : ''}>
			<a href="${ctx}/showcase/product/product?search.show_eq=false"> <i
				class="icon-table"></i> 隐藏的产品列表
		</a>
		</li>
		<c:if test="${not empty param.fromCategory}">
			<li><a href="<es:BackURL/>"> <i class="icon-reply"></i>
					返回类别列表
			</a></li>
		</c:if>
	</ul>
