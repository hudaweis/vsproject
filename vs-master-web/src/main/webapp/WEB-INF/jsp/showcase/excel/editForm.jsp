	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="showcase:excel:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/excel/create?BackURL=<es:BackURL/>"> <i
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/showcase/excel/${m.id}?BackURL=<es:BackURL/>"> <i
					class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="showcase:excel:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/excel/${m.id}/update?BackURL=<es:BackURL/>">
			...
			<shiro:hasPermission name="showcase:excel:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/excel/${m.id}/delete?BackURL=<es:BackURL/>">
