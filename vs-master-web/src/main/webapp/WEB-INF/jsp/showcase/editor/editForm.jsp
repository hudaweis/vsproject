		...
		<shiro:hasPermission name="showcase:editor:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/editor/create?BackURL=<es:BackURL/>"> <i
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="title"
					cssClass="input-xxlarge validate[required,minSize[2],maxSize[200]]" />
			</div>
		</div>
			...
			<div class="controls">
				<c:choose>
					<c:when test="${op ne '查看'}">
						<form:textarea path="content" cssClass="validate[required]"
							cssStyle="width: 550px;height: 300px;" />
					</c:when>
					<c:otherwise>
                        ${m.content}
                    </c:otherwise>
