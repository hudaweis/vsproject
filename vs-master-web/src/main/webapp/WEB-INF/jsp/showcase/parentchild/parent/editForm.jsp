	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="showcase:parentchild:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/parentchild/parent/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/showcase/parentchild/parent/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="showcase:parentchild:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/parentchild/parent/${m.id}/update?BackURL=<es:BackURL/>">
						<i class="icon-edit"></i> 修改
				</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="showcase:parentchild:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/showcase/parentchild/parent/${m.id}/delete?BackURL=<es:BackURL/>">
		...
		<div>
			...
			<div class="control-group span4">
				...
				<div class="controls input-append date">
					<form:input path="beginDate" data-format="yyyy-MM-dd"
			...
			<div class="control-group span4">
				...
				<div class="controls input-append date">
					<form:input path="endDate" data-format="yyyy-MM-dd"
						placeholder="例如2013-02-07" data-position="bottom-left" />
					<span class="add-on"><i data-time-icon="icon-time"
						data-date-icon="icon-calendar"></i></span>
				</div>
			</div>
		...
		<div class="panel">
			<h4 class="hr tool">
				...
				<div class="btn-group" style="font-weight: normal;">
					<a class="btn btn-create-child no-disabled"> <i
						class="icon-edit"></i> 新增
					</a> <a class='btn btn-update-child' href='javascript:void(0);'> <i
						class="icon-edit"></i> 修改
					</a> <a class="btn btn-delete-child"> <i class="icon-trash"></i> 删除
					</a> <a class='btn btn-copy-child'> <i class="icon-copy"></i>
						以此为模板复制一份
					</a>
				</div>
			</h4>
