		...
		<c:forEach items="${page.content}" var="m">
			<tr>
				<td class="check"><input type="checkbox" name="ids"
					value="${m.id}" root="${m.root}"></td>
				<td><a class="btn btn-link no-padding"
					href="${ctx}/admin/sys/resource/${m.id}">${m.id}</a></td>
				<td><sys:showResourceName id="${m.id}" /></td>
				<td>
					<div style="width: 80px; overflow: hidden" data-toggle="tooltip"
						title="${m.identity}">${m.identity}</div>
				</td>
				<td>
					<div style="width: 80px; overflow: hidden" data-toggle="tooltip"
						title="${m.url}">${m.url}</div>
				</td>
				<td>${m.show ? '是' : '否'}</td>
			</tr>
		</c:forEach>
