...
<div class="panel">
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="sys:role:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/permission/role/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/admin/sys/permission/role/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="sys:role:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/permission/role/${m.id}/update?BackURL=<es:BackURL/>">
						<i class="icon-edit"></i> 修改
				</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="sys:role:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/permission/role/${m.id}/delete?BackURL=<es:BackURL/>">
	...
	<form:form id="editForm" method="post" commandName="m"
		cssClass="form-inline form-horizontal form-small">
		...
		<div id="roleInfo">
			...
			<div class="control-group span4">
				...
				<div class="controls">
					<form:input path="role" cssClass="validate[required]"
			...
			<div id="selectResourcePermission">
				...
				<div id="resourceInfo" class="span3">
					...
					<div class="input-append" title="选择资源">
						...
						<input type="text" id="resourceName" class="input-medium"
							value="${target.name}" readonly="readonly"> <a
							id="selectResourceTree" href="javascript:;"> <span
							class="add-on"><i class="icon-chevron-down"></i></span>
						</a>
					</div>
				</div>

				<div id="permissionInfo" class="span4">
					<label>权限</label>&nbsp; <select id="permissions"
						multiple="multiple">
				...
				<div id="appendResourcePermissionBtn" class="control-group span2">
					&nbsp; <a class="btn btn-warning btn-add-resource-permission">
						<i class="icon-file-alt"></i> 添加
					</a>
				</div>
				<div class="muted font-12"
					style="margin: 10px auto; float: left; width: 100%;">
					注意：添加的数据是临时的，还需要点击页面下方的[新增/修改]保存该数据</div>
			</div>

			<div id="selectedResourcePermssion" class="span9">
				<div class="muted font-12" style="margin: 10px auto;">
					已选择的资源和权限 &nbsp;&nbsp;&nbsp;&nbsp; <a
						class="btn btn-link btn-delete-all-resource-permission">删除选中</a>
				</div>
						...
						<c:forEach items="${m.resourcePermissions}" var="o">
							<c:if
								...
								<tr>
									...
									<td><a
										class='btn btn-link btn-edit btn-delete-resource-permission'
