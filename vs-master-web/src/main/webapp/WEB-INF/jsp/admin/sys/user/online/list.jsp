...
<div data-table="table" class="panel" id="panel">
	...
	<ul class="nav nav-tabs">
		<li
			${empty param['search.userId_eq'] and empty param['search.userId_gt'] ? 'class="active"' : ''}>
			<a href="${ctx}/admin/sys/user/online"> <i class="icon-table"></i>
				所有用户列表
		</a>
		</li>

		<li ${not empty param['search.userId_gt'] ? 'class="active"' : ''}>
			<a href="${ctx}/admin/sys/user/online?search.userId_gt=0"> <i
				class="icon-table"></i> 登录用户列表
		</a>
		</li>
		<li ${not empty param['search.userId_eq'] ? 'class="active"' : ''}>
			<a href="${ctx}/admin/sys/user/online?search.userId_eq=0"> <i
				class="icon-table"></i> 匿名游客列表
		</a>
		</li>
	</ul>
		...
		<div class="span4">
			<div class="btn-group">
				<shiro:hasPermission name="sys:userOnline or monitor:userOnline">
					<%-- 等价于sys:userOnline:* 所有权限 --%>
					<a class="btn btn-force-logout"> <span class="icon-lightbulb"></span>
						强制退出
					</a>
				</shiro:hasPermission>
			</div>
