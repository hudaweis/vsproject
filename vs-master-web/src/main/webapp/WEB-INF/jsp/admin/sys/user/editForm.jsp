	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="sys:user:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/user/create?BackURL=<es:BackURL/>"> <i
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/admin/sys/user/${m.id}?BackURL=<es:BackURL/>"> <i
					class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="sys:user:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/user/${m.id}/update?BackURL=<es:BackURL/>">
						<i class="icon-edit"></i> 修改
				</a></li>
			</shiro:hasPermission>
			<shiro:hasPermission name="sys:user:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/admin/sys/user/${m.id}/delete?BackURL=<es:BackURL/>">
		...
		<div id="baseinfo">
			...
			<div class="control-group span4">
				...
				<div class="controls">
					<form:input path="username"
						cssClass="validate[required,custom[username],ajax[ajaxCall]]"
			...
			<div class="control-group span4">
				...
				<div class="controls input-append date">
					<form:input path="createDate" data-format="yyyy-MM-dd hh:mm:ss"
						data-position="bottom-left" placeholder="默认当前时间" />
					...
					<div class="control-group span4">
						...
						<div class="controls">
							<form:password path="password"
								cssClass="validate[required,minSize[5],maxSize[100]]"
			...
			<div class="control-group span4">
				...
				<div class="controls inline-radio">
					<form:radiobuttons path="admin" items="${booleanList}"
			...
			<div class="control-group span4">
				...
				<div class="controls inline-radio">
					<form:radiobuttons path="status" items="${statusList}"
						itemLabel="info" cssClass="validate[required]" />
				</div>
			</div>
		</div>

		<c:set var="displayOrganizationJobs"
			value="${m.displayOrganizationJobs}" />
		<%@include
