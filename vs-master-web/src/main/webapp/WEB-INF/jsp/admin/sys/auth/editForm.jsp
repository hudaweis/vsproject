...
<div class="panel">
		...
		<shiro:hasPermission name="sys:auth:create">
			<c:if test="${op eq '新增'}">
				<c:forEach items="${types}" var="t">
					<li ${t eq type ? 'class="active"' : ''}><a
						href="${ctx}/admin/sys/auth/${t}/create?BackURL=<es:BackURL/>">
							<i class="icon-file-alt"></i> 新增${t.info}授权
					</a></li>
				</c:forEach>
			</c:if>
		</shiro:hasPermission>
	...
	<form:form id="editForm" method="post" commandName="m"
		cssClass="form-inline form-horizontal form-medium">
		...
		<c:if test="${op ne '新增'}">
			...
			<c:if test="${type eq 'user_group'}">
				<div class="control-group">
					<label class="control-label"> 用户分组 </label>
					<div class="controls" style="padding-top: 3px;">
			...
			<c:if test="${type eq 'organization_group'}">
				<div class="control-group">
					<label class="control-label"> 组织机构分组 </label>
					<div class="controls" style="padding-top: 3px;">
						<sys:showGroupName id="${m.groupId}" />
					</div>
				</div>
		...
		<c:if test="${op eq '新增'}">
			<c:if test="${type eq 'user'}">
				<div class="control-group">
					<label for="userIds" class="control-label"> 用户编号(批量)<br />
						<span class="muted">多个逗号分隔</span>
					</label>
					<div class="controls">
						<div style="float: left;">
							<textarea id="userIds" name="userIds" rows="3" cols="20"
								class="validate[required]" placeholder="请输入用户编号，多个逗号分隔"></textarea>
						</div>
						<div id="search-username" class="accordion-body collapse"
							style="float: left; margin-left: 20px">
							<input type="text" id="username" class="input-medium"
								placeholder="输入用户名 检索用户编码">
						</div>

						<span style="margin-left: 20px; display: inline-block"
							data-toggle="tooltip" data-placement="bottom" data-html="true"
							title="根据用户名检索用户编号！<br/>检索到的用户编号会自动添加到【用户编号(批量)】中"> <a
							class="btn btn-link" data-toggle="collapse"
							href="#search-username"> <i class="icon-search"></i>
						</a>
						</span>
					</div>
				</div>
			...
			<c:if test="${type eq 'user_group' or type eq 'organization_group'}">
				<div class="control-group">
					...
					<div class="controls">
						<div style="float: left;">
							<textarea id="groupIds" name="groupIds" rows="3" cols="20"
								class="validate[required]" placeholder="请输入分组编号，多个逗号分隔"></textarea>
						</div>
						<div id="search-groupName" class="accordion-body collapse"
							style="float: left; margin-left: 20px">
							<input type="text" id="groupName" class="input-medium"
								placeholder="输入分组名称 检索分组编码">
						</div>

						<span style="margin-left: 20px; display: inline-block"
							data-toggle="tooltip" data-placement="bottom" data-html="true"
							title="根据分组名检索分组编号！<br/>检索到的分组编号会自动添加到【分组编号(批量)】中"> <a
							class="btn btn-link" data-toggle="collapse"
							href="#search-groupName"> <i class="icon-search"></i>
						</a>
						</span>
					</div>
				</div>
			...
			<c:if test="${type eq 'organization_job'}">
				...
				<%@include
					file="/WEB-INF/jsp/admin/sys/organization/selectOrganizationAndJob.jspf"%>
			</c:if>
		</c:if>
		...
		<div class="control-group">
			...
			<div class="controls">
				<div class="auth">
							...
							<ul>
								<c:forEach items="${roles}" var="r">
									<c:if test="${not esfn:in(m.roleIds, r.id)}">
										<li class="ui-state-default" data-value="${r.id}"
											title="${r.description}">${r.name}[${r.role}]</li>
									</c:if>
								</c:forEach>
							</ul>
						</div>
					...
					<div class="btns">
						<a class="btn btn-link btn-move-all-right" data-toggle="tooltip"
							data-placement="bottom" title="全部右移"> <i
							class="icon-double-angle-right"></i>
						</a> <a class="btn btn-link btn-move-all-left" data-toggle="tooltip"
							data-placement="bottom" title="全部左移"> <i
