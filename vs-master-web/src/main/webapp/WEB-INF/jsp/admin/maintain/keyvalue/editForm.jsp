...
<div class="panel">
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="maintain:keyvalue:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/keyvalue/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/admin/maintain/keyvalue/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="maintain:keyvalue:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/keyvalue/${m.id}/update?BackURL=<es:BackURL/>">
			...
			<shiro:hasPermission name="maintain:keyvalue:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/keyvalue/${m.id}/delete?BackURL=<es:BackURL/>">
	...
	<form:form id="editForm" method="post" commandName="m"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="key"
					cssClass="validate[required,ajax[ajaxKeyCall]]" />
			</div>
		</div>
