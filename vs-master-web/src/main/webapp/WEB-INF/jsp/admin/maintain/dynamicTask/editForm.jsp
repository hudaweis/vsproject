...
<div class="panel">
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="maintain:dynamicTask:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/dynamicTask/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/admin/maintain/dynamicTask/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="maintain:dynamicTask:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/dynamicTask/${m.id}/update?BackURL=<es:BackURL/>">
			...
			<shiro:hasPermission name="maintain:dynamicTask:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/dynamicTask/${m.id}/delete?BackURL=<es:BackURL/>">
	...
	<form:form id="editForm" method="post" commandName="m"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="cron" cssClass="validate[required]"
					placeholder="如 0 30 2 * * ?" />
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="methodName" cssClass="validate[required]"
					placeholder="任务方法名" />
			</div>
		</div>
