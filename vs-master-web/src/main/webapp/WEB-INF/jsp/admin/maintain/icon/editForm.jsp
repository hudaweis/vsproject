...
<div class="panel">
		...
		<shiro:hasPermission name="maintain:icon:create">
			<c:if test="${op eq '新增'}">
				<c:forEach items="${types}" var="t">
					<li ${type eq t ? 'class="active"' : ''}><a
						href="${ctx}/admin/maintain/icon/${t}/create?BackURL=<es:BackURL/>">
							<i class="icon-file-alt"></i> 新增${t.info}
					</a></li>
				</c:forEach>
			</c:if>
		</shiro:hasPermission>
	...
	<form:form id="editForm" method="post" commandName="m"
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="identity"
					cssClass="validate[required,custom[identity],ajax[ajaxIdentityCall]]"
					placeholder="用于表示图标的唯一标识" />
			</div>
		</div>
		<c:if test="${type eq 'css_class'}">
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="cssClass" cssClass="validate[required]"
						placeholder="图标css类" />
				</div>
			</div>
		</c:if>
		...
		<c:if test="${type eq 'css_sprite'}">
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="spriteSrc" />
					<span class="muted font-12">绝对路径：如http://www.vshuok.com，相对路径不要加上下文</span>
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="left"
						cssClass="validate[required, custom[integer]] input-small"
						placeholder="距离左边" data-toggle="tooltip" data-placement="bottom"
						title="距离左边" />
					<form:input path="top"
						cssClass="validate[required, custom[integer]] input-small"
						placeholder="距离上边" data-toggle="tooltip" data-placement="bottom"
						title="距离上边" />
				</div>
			</div>
			...
			<div class="control-group">
				...
				<div class="controls">
					<form:input path="width"
						cssClass="validate[required, custom[integer]] input-small"
						placeholder="宽度" data-toggle="tooltip" data-placement="bottom"
						title="宽度" />
					<form:input path="height"
						cssClass="validate[required, custom[integer]] input-small"
						placeholder="高度" data-toggle="tooltip" data-placement="bottom"
						title="高度" />
				</div>
			</div>
			<div class="control-group">
