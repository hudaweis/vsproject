	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="maintain:notificationTemplate:create">
			<c:if test="${op eq '新增'}">
				<li ${op eq '新增' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/notification/template/create?BackURL=<es:BackURL/>">
		...
		<c:if test="${not empty m.id}">
			<li ${op eq '查看' ? 'class="active"' : ''}><a
				href="${ctx}/admin/maintain/notification/template/${m.id}?BackURL=<es:BackURL/>">
					<i class="icon-eye-open"></i> 查看
			</a></li>
			<shiro:hasPermission name="maintain:notificationTemplate:update">
				<li ${op eq '修改' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/notification/template/${m.id}/update?BackURL=<es:BackURL/>">
			...
			<shiro:hasPermission name="maintain:notificationTemplate:delete">
				<li ${op eq '删除' ? 'class="active"' : ''}><a
					href="${ctx}/admin/maintain/notification/template/${m.id}/delete?BackURL=<es:BackURL/>">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="name"
					cssClass="validate[required,custom[name],ajax[ajaxNameCall]]"
