...
<div class="panel">
	...
	<ul class="nav nav-tabs">
		<c:if test="${op eq '发送草稿'}">
			<li class="active"><a
				href="${ctx}/admin/personal/message/draft/${m.id}/send?BackURL=<es:BackURL/>">
					<i class="icon-file-alt"></i> ${op}
			</a></li>
		</c:if>
		<c:if test="${op eq '发送新消息'}">
			<li class="active"><a
				href="${ctx}/admin/personal/message/send?BackURL=<es:BackURL/>">
					<i class="icon-file-alt"></i> ${op}
			</a></li>
		</c:if>
		<c:if test="${op eq '回复消息'}">
			<li class="active"><a
				href="${ctx}/admin/personal/message/${parent.id}/reply?BackURL=<es:BackURL/>">
					<i class="icon-file-alt"></i> ${op}
			</a></li>
		</c:if>
		<c:if test="${op eq '转发消息'}">
			<li class="active"><a
				href="${ctx}/admin/personal/message/${parent.id}/forward?BackURL=<es:BackURL/>">
					<i class="icon-file-alt"></i> ${op}
			</a></li>
		</c:if>
	...
	<c:if test="${op eq '回复消息'}">
		<div class="control-group">
			<span class="bold span2"><pretty:prettyTime
					date="${parent.sendDate}" /></span>
			<c:choose>
			...
			<div class="accordion message">
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle bold no-underline"
							data-toggle="collapse" href="#collapse${parent.id}">
							${parent.title} <span class="muted"
							style="float: right; padding-right: 20px;"> <sys:showUsername
									id="${parent.senderId}" needLink="false" /> &nbsp;&nbsp;&nbsp;
					...
					<div id="collapse${parent.id}" class="accordion-body collapse in">
						<div class="accordion-inner">${parent.content.content}</div>
					</div>
				</div>
				<br />
	</c:if>
	<form:form id="editForm" method="post" commandName="m"
		cssClass="form-horizontal form-small">
		...
		<c:if test="${op eq '发送新消息' or op eq '转发消息' or op eq '发送草稿'}">
			<div class="control-group">
				...
				<div id="search-receiver" class="controls">
					<c:set var="receiver"
						value="${not empty receiver ? receiver : param.receiver}" />
					<input type="text" id="receiverId_msg" name="receiver"
						value="${receiver}" placeholder="输入收件人用户名">
					<form:hidden path="receiverId" />
				</div>
			</div>
		</c:if>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="title"
					cssClass="validate[required,minSize[5],maxSize[200]]"
					placeholder="长度在5-200个字符之间" />
			</div>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:textarea id="content" path="content.content"
					data-promptPosition="topRight:-200"
					cssStyle="width: 680px;height: 300px;" />
			</div>
		</div>
		...
		<div class="control-group">
			<div class="controls">
				...
				<a class="btn btn-save-draft"> <i class="icon-save"></i> 存草稿
				</a>
			</div>
		</div>
