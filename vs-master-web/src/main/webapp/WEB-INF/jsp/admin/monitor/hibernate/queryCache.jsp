...
<div data-table="table" class="panel">
	...
	<ul class="nav nav-pills">
		...
		<li ${param.sort eq 'hitPercent' ? "class='active'" : ""}><a
			href="?sort=hitPercent">总命中率</a></li>
		<li ${param.sort eq 'executionCount' ? "class='active'" : ""}><a
			href="?sort=executionCount">执行次数</a></li>
		<li ${param.sort eq 'executionAvgRowCount' ? "class='active'" : ""}><a
			href="?sort=executionAvgRowCount">执行返回的平均行数</a></li>
		<li ${param.sort eq 'executionAvgTime' ? "class='active'" : ""}><a
			href="?sort=executionAvgTime">平均执行时间</a></li>
		<li ${param.sort eq 'executionMaxTime' ? "class='active'" : ""}><a
			href="?sort=executionMaxTime">最长执行时间</a></li>
	</ul>
	<table class="table table-bordered">
		<tbody>
			<%
        ...
        %>
			...
			<c:set var="queryCacheCount"
				value="${queryCacheCount > 0 ? queryCacheCount : 1}" />
			<c:set var="queryCacheHitPercent"
				value="${queryStatistics.cacheHitCount * 1.0 / (queryCacheCount)}" />
			...
			<tr>
				<td>命中次数:</td>
				<td>${queryStatistics.cacheHitCount}&nbsp;&nbsp; 平均:<fmt:formatNumber
						value="${queryStatistics.cacheHitCount/upSeconds}"
						maxFractionDigits="2" />次/秒
				</td>
			</tr>
			<tr>
				<td>失效次数:</td>
				<td>${queryStatistics.cacheMissCount}&nbsp;&nbsp; 平均:<fmt:formatNumber
						value="${queryStatistics.cacheMissCount/upSeconds}"
						maxFractionDigits="2" />次/秒
				</td>
			</tr>
			<tr>
				<td>往缓存中Put总次数:</td>
				<td>${queryStatistics.cachePutCount}</td>
			</tr>
			<tr>
				<td style="width: 30%">执行次数（实际查数据库）:</td>
				<td>${queryStatistics.executionCount}</td>
			</tr>
			<tr>
				<td>执行返回行数:</td>
				<td>总行数:${queryStatistics.executionRowCount} <c:set
						var="executionAvgRowCount"
						value="${queryStatistics.executionRowCount/queryStatistics.executionCount}" />
					<c:set var="executionAvgRowCount"
						value="${executionAvgRowCount > 0 ? executionAvgRowCount : 0}" />
			...
			<tr>
				<td>执行时间:</td>
				<td>平均:${queryStatistics.executionAvgTime}毫秒
					最长:${queryStatistics.executionMaxTime}毫秒
					最短:${queryStatistics.executionMinTime}毫秒</td>
			</tr>
