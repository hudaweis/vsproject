...
<%@ attribute name="fileInfo" type="java.util.Map" required="false"
	description="当前文件"%>
<%@ attribute name="checkbox" type="java.lang.Boolean" required="false"
	description="是否需要checkbox，默认true"%>
...
<tr>
	<td class="check"><c:if test="${checkbox}">
	...
	<td style="word-break: break-all;"><c:choose>
			<c:when test="${fileInfo.isDirectory}">
				<a href="${ctx}/admin/maintain/editor/list?path=${fileInfo.path}">
					<i class="${fileInfo.iconSkin}"></i> ${fileInfo.name}
				</a>
			</c:when>
