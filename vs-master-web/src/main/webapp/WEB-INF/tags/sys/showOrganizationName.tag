...
<%@ tag import="com.vshuok.es.common.utils.SpringUtils"%>
<%@ tag import="com.vshuok.es.sys.organization.entity.Organization"%>
<%@ tag
...
<%@ attribute name="id" type="java.lang.Long" required="true"
	description="当前要展示的组织机构的名字"%>
<%@ attribute name="showParents" type="java.lang.Boolean"
	required="false" description="是否显示父亲 默认包含"%>
<%@ attribute name="includeRoot" type="java.lang.Boolean"
	required="false" description="是否包含根 默认true"%>
...
<%
	...
	if (organization != null) {
		...
		if (showParents == true) {
			List<Organization> parents = organizationService
					.findAncestor(organization.getParentIds());
	...
	s.append(String
			.format("<a class='btn btn-link no-padding' href='%s/admin/sys/organization/organization/%d'>",
