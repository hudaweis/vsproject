<%@ tag import="com.vshuok.es.common.utils.ImagesUtils"%>
...
<%@ attribute name="filename" type="java.lang.String" required="true"
	description="附件地址"%>
<%@ attribute name="noAttachmentTitle" type="java.lang.String"
	required="false" description="没有附件时显示的标题"%>
<%@ attribute name="isDownload" type="java.lang.Boolean"
	required="false" description="是否直接下载，默认false"%>
<%@ attribute name="showImage" type="java.lang.Boolean" required="false"
	description="如果是图片是否显示"%>
<%@ attribute name="width" type="java.lang.String" required="false"
	description="图片的高度，默认160"%>
<%@ attribute name="height" type="java.lang.String" required="false"
	description="图片的宽度，默认120"%>

<%@ attribute name="left" type="java.lang.String" required="false"
	description="css sprite 距离左边"%>
<%@ attribute name="top" type="java.lang.String" required="false"
	description="css sprite 距离上边"%>
	...
	<c:when test="${not empty filename}">
		<%
			String displayFileName = FilenameUtils.getName(filename);
					displayFileName = displayFileName.substring(displayFileName
							.indexOf("_") + 1);
					...
					if (showImage && ImagesUtils.isImage(filename)) {
						...
						boolean isCssSprite = StringUtils.isNotEmpty(left)
								&& StringUtils.isNotEmpty(top);
						...
						String template = templatePrefix
								+ "<img src='%1$s/%2$s' style='display:inline-block;vertical-align: middle;height:%3$spx;width:%4$spx;%5$s'/>"
								+ templateSuffix;
						if (isCssSprite) {
						...
						if (isCssSprite) {
							out.write(String.format(template, ctx, filename,
									left, top, height, width, style));
						} else {
							out.write(String.format(template, ctx, filename,
									height, width, style));
						}
