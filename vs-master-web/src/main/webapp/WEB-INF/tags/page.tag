...
<div
	class="table-pagination <c:if test='${simple ne false}'> row-fluid tool ui-toolbar</c:if>">
	...
	<div class="pagination">
		...
		<div>
			<span class="page-input"> 第<input type="text"
				class="input-mini" value="${current}"
				onblur="$.table.turnPage('${pageSize}', $(this).val(), this);" />页
			</span> &nbsp; <select class="input-small"
				onchange="$.table.turnPage($(this).val(), ${current}, this);">
				<option value="10"
			...
			</select> <span class="page-info">[共${page.totalPages}页/${page.totalElements}条]</span>
		</div>
	</div>
</div>
