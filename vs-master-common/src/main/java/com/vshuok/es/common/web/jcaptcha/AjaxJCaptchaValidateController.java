package com.vshuok.es.common.web.jcaptcha;
...
public class AjaxJCaptchaValidateController {
	...
	public Object jqueryValidationEngineValidate(
		...
		if (JCaptcha.hasCaptcha(request, fieldValue) == false) {
			response.validateFail(fieldId, messageSource.getMessage(
					"jcaptcha.validate.error", null, null));
		} else {
			response.validateSuccess(fieldId, messageSource.getMessage(
					"jcaptcha.validate.success", null, null));
		}
