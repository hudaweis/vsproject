...
public abstract class BaseTreeableService<M extends BaseEntity<ID> & Treeable<ID>, ID extends Serializable>
	...
	public void move(M source, M target, String moveType) {
		...
		if ("next".equals(moveType)) {
			...
			if (siblings.isEmpty()) { // 如果没有兄弟了 则直接把源的设置为目标即可
	 ...
	 * @return
	 */
	public Set<String> findNames(Searchable searchable, String name,
			ID excludeId) {
		...
		return Sets.newHashSet(Lists.transform(
				findAll(searchable).getContent(), new Function<M, String>() {
					@Override
	...
	public List<M> findChildren(List<M> parents, Searchable searchable) {
		...
		SearchFilter first = SearchFilterHelper.newCondition("parentIds",
				SearchOperator.prefixLike, parents.get(0)
						.makeSelfAsNewParentIds());
		SearchFilter[] others = new SearchFilter[parents.size() - 1];
		for (int i = 1; i < parents.size(); i++) {
			others[i - 1] = SearchFilterHelper.newCondition("parentIds",
					SearchOperator.prefixLike, parents.get(i)
							.makeSelfAsNewParentIds());
		}
	...
	public List<M> findAncestor(String parentIds) {
		...
		return Lists.reverse(findAllWithNoPageNoSort(Searchable.newSearchable()
