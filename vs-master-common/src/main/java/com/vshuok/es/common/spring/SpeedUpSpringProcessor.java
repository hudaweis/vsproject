package com.vshuok.es.common.spring;
...
public class SpeedUpSpringProcessor implements BeanFactoryPostProcessor {
	...
	private final Logger log = LoggerFactory
			.getLogger(SpeedUpSpringProcessor.class);
	 ...
	 */
	public void setRemoveOrReplaceBeanProperties(
			String[] removeOrReplaceBeanProperties) {
		this.removedBeanProperties = new HashMap<String, Set<String[]>>(
				removeOrReplaceBeanProperties.length);
		for (String removeOrReplaceBeanProperty : removeOrReplaceBeanProperties) {
			...
			if (isReplace) {
				removeBeanProperty = removeBeanProperty
			...
			Set<String[]> propertiesSet = this.removedBeanProperties
					.get(beanName);
			...
			propertiesSet.add(Arrays.copyOfRange(properties, 1,
					properties.length));
	...
	private boolean needRemove(String beanName, BeanDefinition beanDefinition) {
					...
					if (properties.length == 1) {
						...
						if (this.replaceBeanProperties != null) {
							...
							if (this.replaceBeanProperties.containsKey(key)) {
								propertyValues.add(properties[0],
						...
						if (propertyValue != null) {
							...
							if (nextValue instanceof ManagedMap) {
								...
								((ManagedMap) nextValue)
										.remove(typedStringValue);

								// 如果需要替换，替换掉值（只支持基本属性）
								if (this.replaceBeanProperties != null) {
									String key = beanName + "@" + properties[0]
											+ "@" + properties[1];
									if (this.replaceBeanProperties
											.containsKey(key)) {
										((ManagedMap) nextValue).put(
												properties[1],
												this.replaceBeanProperties
														.get(key));
									}
								}
							}
		...
		// spring data jpa
		if (className
				.equals("com.vshuok.es.common.repository.support.SimpleBaseRepositoryFactoryBean")
				|| className
						.equals("org.springframework.data.jpa.repository.support.JpaRepositoryFactoryBean")) {
			PropertyValue repositoryInterfaceValue = beanDefinition
					.getPropertyValues()
